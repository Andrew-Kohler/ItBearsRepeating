    ...
    void Update()
    {
        if (canInteract)
        {
            if (Input.GetButtonDown("Interact"))
            {
                ...
                player.GetComponent<Health>().RestoreHealth(player.GetComponent<Health>().maxHealth / 4);
                player.GetComponent<PlayerSprint>().RestoreStamina(player.GetComponent<PlayerSprint>().maxStamina / 4);
